...
//
//	virtual bool CreateDefaultContent(ID3D11Device* device, const char* filePath, ContentManager* contentManager) override;
//
//protected:
//	DiskContent* CreateInstance() const;
//	bool Apply(Content* content);
//
//private:
//	const unsigned int errorCharacterID = 0x3F; //0x3F = "?"
//
//	CONTENT_ERROR_CODES Load(ID3D11Device* device, const char* filePath, ContentManager* contentManager = nullptr, ContentParameters* contentParameters = nullptr) override;
//	CONTENT_ERROR_CODES LoadTemporary(ID3D11Device* device, const char* filePath, ContentManager* contentManager = nullptr) override;
