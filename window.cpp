...
#ifndef _WIN32
GLXContext OSWindow::CreateNewSharedContext() {
    GLXContext shared = CreateContext(GetConfig(), context);
    extra_contexts.emplace_back(shared);
    return shared;
}

GLXContext OSWindow::GetContextByNumber(int i) {
    if(i == 0)
        return context;

    if(extra_contexts.size() <= i)
        return extra_contexts[i-1];

    return nullptr;
}

#else
HGLRC OSWindow::CreateNewSharedContext() {
	HGLRC shared = wglCreateContext(hdc);
	BOOL error = wglShareLists(hglrc, shared);
	if(error == FALSE)
	{
...
HGLRC OSWindow::GetContextByNumber(int i) {
	if (i == 0)
		return hglrc;
