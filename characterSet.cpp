...
CONTENT_ERROR_CODES CharacterSet::Load(ID3D11Device* device, const char* filePath, ContentManager* contentManager /*= nullptr*/, ContentParameters* contentParameters /*= nullptr*/)
{
	unsigned int width;
	unsigned int height;
	...
	TextureCreationParameters textureParameters(uniqueID.c_str(), width, height, DXGI_FORMAT_R8_UNORM, D3D11_BIND_SHADER_RESOURCE, &buffer[0], 8);
	this->texture = contentManager->Load<MemoryTexture>("", &textureParameters);

	return CONTENT_ERROR_CODES::NONE;
}

CONTENT_ERROR_CODES CharacterSet::LoadTemporary(ID3D11Device* device, const char* filePath, ContentManager* contentManager /*= nullptr*/)
{
	unsigned int width;
	unsigned int height;
	...
	TextureCreationParameters textureParameters(uniqueID.c_str(), width, height, DXGI_FORMAT_R8_UNORM, D3D11_BIND_SHADER_RESOURCE, &buffer[0], 8);
	this->texture = contentManager->LoadTemporary<MemoryTexture>("", &textureParameters);
...
std::vector<uint8_t> CharacterSet::CreateBuffer(const char* filePath, unsigned int& width, unsigned int& height)
{
	...
	returnVector.resize(width * height, 0);
	for(const auto& character : characters)
	{
		...
		for(int y = 0, endy = static_cast<int>(character.second.height); y < endy; ++y)
			for(int x = 0, endx = static_cast<int>(character.second.width); x < endx; ++x)
				returnVector[(character.second.y + y) * width + (character.second.x + x)] = slot->bitmap.buffer[y * slot->bitmap.width + x];
	}
...
bool CharacterSet::CreateDefaultContent(ID3D11Device* device, const char* filePath, ContentManager* contentManager)
{
	std::string filePathString(filePath);
	...
	if(Load(device, newFont.c_str(), contentManager, nullptr) == CONTENT_ERROR_CODES::NONE)
		return true;
#else
#ifndef CONTENT_IGNORE_OPERATING_SYSTEM
#error Unsupported operating system, default font loading will not work. Define CONTENT_IGNORE_OPERATING_SYSTEM to ignore this warning.
#endif
#endif
...
bool CharacterSet::Apply(Content* content)
{
	...
	this->texture->Apply(other->texture);
